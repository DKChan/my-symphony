...
Symphony 是一个长期运行的自动化服务，它持续从问题跟踪器（如 GitHub）读取工作，为每个问题创建隔离的工作空间，并在工作空间内运行编码代理会话。
...
- Go 1.22+
- GitHub Token（如使用 GitHub tracker）
- Codex CLI（或兼容的编码代理）
...
2. 设置环境变量（如使用 GitHub tracker）:

```bash
export GITHUB_TOKEN="your-github-token"
```

3. 编辑 `WORKFLOW.md` 中的 `repo` 为你的 GitHub 仓库（格式: owner/repo）。
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `tracker.kind` | 跟踪器类型 | `mock` |
| `tracker.api_key` | API 密钥（支持 `$VAR` 格式） | - |
| `tracker.repo` | GitHub 仓库（格式: owner/repo） | - |
